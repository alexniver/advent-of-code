...
fn apply_move(mut stack_arr: Vec<Vec<u8>>, move_arr: Vec<Move>) -> Vec<Vec<u8>> {
    for m in move_arr {
        let mut pop_arr = Vec::with_capacity(m.num as usize);
        for _ in 0..m.num {
            pop_arr.insert(0, stack_arr[(m.from - 1) as usize].pop().unwrap());
        }

        stack_arr[(m.to - 1) as usize].extend_from_slice(&pop_arr);
    }
    stack_arr
}
