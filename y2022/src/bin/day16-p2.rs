...
fn process(s: &str) -> usize {
    ...
    for (idx, &id) in v_target_list.iter().enumerate() {
        vv_list[id].target_idx = idx;
    }
    println!("size: {:?}", v_target_list.len());

    let all_open = (1 << v_target_list.len()) - 1;
    println!("all_open: {:?}", all_open);
    let mut max: usize = 0;
    for i in 0..=all_open {
        max = max.max(
            dfs(aa_idx, 26, i, &vv_list, &v_target_list, &path_time_dict)
                + dfs(
                    aa_idx,
                    26,
                    all_open ^ i,
