...
fn process(input: &str) -> usize {
    ...
    let mut result = 0;
    let mut z_global = 0;
    let target_arr = [(entry, exit), (exit, entry), (entry, exit)];

    for (entry, exit) in target_arr {
        let mut result_tmp = usize::MAX;

        let state = State::new(entry, 0, z_global);
        ...
        while !queue.is_empty() {
            ...
            if state.pos.x == exit.x && (state.pos.y + 1 == exit.y || state.pos.y - 1 == exit.y) {
                if result_tmp > state.time + 1 {
                    result_tmp = state.time + 1;
                    z_global = state.z + 1;
                }
            }
