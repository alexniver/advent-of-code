...
fn main() {
            ...
            if let Ok(line) = line {
                ...
                if let Some("Monkey") = parts.next() {
                    ...
                    loop {
                        ...
                        monkey.add_item_worry_level(p[0..p.len()].parse::<i64>().unwrap());
                    }
    ...
    system.common_remainder = system.monkey_arr.iter().map(|m| m.div).product();
    ...
    let mut max_arr = [0; 2];
    for m in system.monkey_arr.iter() {
...
impl Monkey {
    ...
    fn add_item_worry_level(&mut self, worry_level: i64) {
        ...
        for i in 0..self.monkey_arr.len() {
            ...
            for _ in 0..m.item_worry_level_arr.len() {
                ...
                remove_arr.push((
                    ...
                    worry_level % self.common_remainder,
                ));
            }

            for remove in remove_arr {
                // let worry_level = remove.1 % self.monkey_arr[remove.0].div;
