...
fn process(input: &str) -> i64 {
    let num_arr_origin = parse(input)
        .unwrap()
        .1
        .iter_mut()
        .map(|v| *v * 811589153)
    ...
    let len = num_arr_origin.len() as i64;
    ...
    for _ in 0..10 {
        for i in 0..len {
            let idx = num_idx_arr.iter().position(|idx| *idx == i).unwrap() as i64;
...
4";
    ...
    #[test]
    fn process_test() {
        assert_eq!(process(INPUT), 1623178306);
    }
}
